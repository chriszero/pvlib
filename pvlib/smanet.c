...
static int con_write(smanet_t *smanet, uint8_t *data, int len, int broadcast)
{
    if (smanet->smabluetooth) {
        ...
        packet.data        = data;
        packet.data_len    = len;
        packet.header.cmd  = 0x01;
        packet.unknown_src = 0;
...
static int con_read(smanet_t *smanet, uint8_t *data, int len)
{
    if (smanet->smabluetooth) {
        smabluetooth_packet_t packet;
        ...
        return connection_read(smanet->con, data, len);
    }
}
...
static int read_frame(smanet_t *smanet, uint8_t *data, int len)
{
    ...
    for (i = 0; (i < FRAME_SIZE) && !sync; i++) {
        ...
        if (((pos + 1 == size) && (smanet->read_buf[pos] != HDLC_SYNC))
                || pos >= size) {
            if (pos >= size) {
                size = con_read(smanet, smanet->read_buf, BUF_SIZE);
                if (size < 0) return -1;
            } else {
                smanet->read_buf[0] = smanet->read_buf[size - 1];
                size = con_read(smanet, &smanet->read_buf[1], BUF_SIZE - 1);
                if (size < 0) return -1;
                size++;
            }
            pos = 0;
        ...
        case HDLC_SYNC :
            if (i == 0) {
                ...
                while ((pos < size) && (smanet->read_buf[pos] == HDLC_SYNC)) pos++;
                i--;
            } else {
                i--;
...
int smanet_read(smanet_t *smanet, uint8_t *data, int len)
{
    return read_frame(smanet, data, len);
}

int smanet_write(smanet_t *smanet, uint8_t *data, int len, int broadcast)
{
    uint8_t  buf[FRAME_SIZE];
    uint16_t fcs;
    uint16_t pos = 0;
    ...
    LOG_HEX("smanet write", buf, pos);
    return con_write(smanet, buf, pos, broadcast);
}
